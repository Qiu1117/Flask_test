...
@dashboard.route("/dashboard_dataset_patient_count", methods=["GET"])
def get_dataset_patient_count():
    try:
        query = (
            db.session.query(
                Dataset.id,
...
def get_dashboard_dataset_info():
    ...
    if dataset_id:
        ...
        instances = (
            db.session.query(func.count(Dataset_Instances.instance_orthanc_id.distinct()))
...
@dashboard.route("/dashboard_dataset_patient_sex_count", methods=["GET"])
def get_dataset_patient_sex_count():
    try:
        query = (
            db.session.query(
                Dataset.id,
...
def get_patient_gender_count():
    try:
        query = (
            ...
            .join(
                Dataset_Patients,
                Patient.patient_orthanc_id == Dataset_Patients.patient_orthanc_id,
            )
            .filter(Patient.valid == True, Dataset_Patients.valid == True)
            .group_by(Patient.patient_sex)
            .all()
        )

        result = {sex if sex else "Unknown": count for sex, count in query}
...
def get_patient_age_distribution():
    try:
        ...
        for group, count in age_groups.items():
            ...
            age_distribution[f"{group}-{group+9}"] = round(percentage, 2)
